...
const CONFIG = {
    ...
    BLOCKED_APP_IDS: new Set([
        ...
        // 在这里添加其他需要屏蔽链接的 App ID (字符串格式)
        // 例如: '另一个AppID', '再一个AppID'
    ])
};
...
async function handleChartQuery(regionName, chartType) {
    ...
    try {
        ...
        apps.forEach((app, index) => {
            const appId = app.id; // Apple RSS Feed 通常用 'id' 字段表示 App ID (trackId)
            const appName = app.name || '未知应用'; // 添加默认值以防万一
            const appUrl = app.url;

            // 【核心修改】检查当前 App ID 是否在屏蔽列表中
            if (CONFIG.BLOCKED_APP_IDS.has(String(appId))) { // 强制转为字符串比较
                // 如果在屏蔽列表，只显示名称，不加链接
                resultText += `${index + 1}、${appName}\n`;
            } else if (appUrl) { // 确保 URL 存在才添加链接
                // 如果不在屏蔽列表，且 URL 有效，则显示带链接的名称
                resultText += `${index + 1}、<a href="${appUrl}">${appName}</a>\n`;
            } else {
                 // 如果 URL 无效，也只显示名称
                 resultText += `${index + 1}、${appName}\n`;
            }
        ...
        console.error("Error in handleChartQuery:", e.message); // 添加错误日志
        return '获取榜单失败，请稍后再试。';
    }
}
